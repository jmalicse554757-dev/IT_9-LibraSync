...
<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:22px;">
    <div>
        ...
        <p style="color:var(--text-muted);font-size:13px;">Personal and account information</p>
    </div>
...
<div style="background:var(--maroon-deep);border-radius:14px;padding:24px 28px;margin-bottom:20px;display:flex;align-items:center;gap:20px;">
    ...
    <div>
        ...
        <div style="font-size:12px;color:rgba(255,255,255,0.6);margin-top:3px;text-transform:uppercase;letter-spacing:1px;">Librarian · {{ $user->program ?? 'Library Staff' }}</div>
        <div style="font-size:12px;color:rgba(255,255,255,0.5);margin-top:2px;">ID: {{ $user->employee_id ?? 'N/A' }}</div>
    </div>
</div>
...
<div class="grid-2">
    ...
    <div class="card">
        ...
        <table style="width:100%;border-collapse:collapse;">
            ...
            <tr>
                ...
                <td style="padding:10px 0;font-size:13px;color:var(--text-dark);">{{ $user->date_of_birth ? $user->date_of_birth->format('F d, Y') : 'N/A' }}</td>
            </tr>
        </table>
    </div>
    ...
    <div class="card">
        ...
        <table style="width:100%;border-collapse:collapse;">
            <tr style="border-bottom:1px solid rgba(232,213,196,0.4);">
                <td style="padding:10px 0;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:var(--text-muted);width:40%;">Employee ID</td>
                <td style="padding:10px 0;font-size:13px;font-weight:700;color:var(--red-main);">{{ $user->employee_id ?? 'N/A' }}</td>
            </tr>
            <tr style="border-bottom:1px solid rgba(232,213,196,0.4);">
                <td style="padding:10px 0;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:var(--text-muted);">Position</td>
                <td style="padding:10px 0;font-size:13px;color:var(--text-dark);">{{ $user->program ?? 'Librarian' }}</td>
            </tr>
            <tr style="border-bottom:1px solid rgba(232,213,196,0.4);">
                <td style="padding:10px 0;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:var(--text-muted);">Role</td>
                <td style="padding:10px 0;font-size:13px;color:var(--text-dark);">{{ ucfirst($user->role) }}</td>
            </tr>
            <tr style="border-bottom:1px solid rgba(232,213,196,0.4);">
            ...
            <tr>
                <td style="padding:10px 0;font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:var(--text-muted);">Member Since</td>
                <td style="padding:10px 0;font-size:13px;color:var(--text-dark);">{{ $user->created_at->format('F d, Y') }}</td>
            </tr>
        </table>
    </div>
</div>
...
<div class="modal-overlay" id="editProfileModal">
    <div class="modal" style="max-width:520px;">
        ...
        <form method="POST" action="{{ route('librarian.profile.update') }}" enctype="multipart/form-data">
            @csrf
        ...
        <form method="POST" action="{{ route('librarian.profile.password') }}">
            @csrf
