...
<div style="margin-bottom:22px;">
    ...
    <p style="color:var(--text-muted);font-size:13px;">Post and manage announcements for students and librarians</p>
</div>
...
<div style="display:grid;grid-template-columns:1fr 1.6fr;gap:20px;align-items:start;">
        ...
        <form method="POST" action="{{ route('admin.announcements.store') }}">
            ...
            <div style="margin-bottom:14px;">
                ...
                <select name="audience" required
                    ...
                    <option value="librarian" {{ old('audience') === 'librarian' ? 'selected' : '' }}>Librarians Only</option>
                </select>
            </div>
    ...
    <div>
        <div style="font-family:'Playfair Display',serif;font-size:17px;font-weight:700;color:var(--maroon-deep);margin-bottom:14px;display:flex;align-items:center;gap:8px;">
            ...
            <span style="font-size:12px;font-weight:700;padding:3px 10px;border-radius:20px;background:var(--maroon-deep);color:#fff;">{{ $announcements->count() }}</span>
        </div>
                ...
                <div style="padding:16px;">
                    ...
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-top:10px;padding-top:10px;border-top:1px solid var(--border);">
                        <div style="font-size:11px;color:var(--text-muted);">
                            Posted by <strong>{{ $a->author?->full_name ?? 'Admin' }}</strong> · {{ $a->created_at->format('M d, Y h:i A') }}
                        </div>
        ...
        <form method="POST" action="{{ route('admin.announcements.update', $a) }}">
            ...
            <div style="margin-bottom:14px;">
                ...
                <select name="audience" required
                    ...
                    <option value="librarian" {{ $a->audience === 'librarian' ? 'selected' : '' }}>Librarians Only</option>
                </select>
            </div>
            <div style="margin-bottom:16px;">
                <label style="font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:var(--text-muted);display:block;margin-bottom:6px;">Message</label>
